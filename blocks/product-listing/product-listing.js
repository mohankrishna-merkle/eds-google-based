...
async function fetchIndex(source) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Product listing: failed to load index', source, response.status);
    ...
    console.debug('Product listing: fetched index from', source, json?.data?.length, 'items');
    ...
    console.error('Product listing: error fetching index', source, err);
...
function filterProducts(data, query, category) {
  ...
  // FIX: removed the price/path filter that was silently dropping

  // pages that hadn't yet had price metadata added.

  // We only require path and title to be present.
...
export default async function decorate(block) {
  ...
  // FIX: default source now points to /products/query-index.json

  // Can be overridden in the block config via 'index-source' key.

  // Example block table row: | index-source | /products/query-index.json |

  const source = config['index-source']
  ...
  if (!data?.length) {
    ...
    status.textContent = 'No products found. Publish product pages and ensure the index is configured.';

    return;

  }
 
  // FIX: removed the price/path filter here — use all items from the index.

  // The index include path (/products/**) already scopes what gets indexed.

  // Filtering by price here silently hides products without a price set yet.

  const productItems = data.filter((item) => item.path && item.title);
 
  if (!productItems.length) {

    status.textContent = 'No products found. Ensure product pages have a title and are published.';
